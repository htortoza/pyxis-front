...
import type { DimensionKey } from '../data/models/tenant-vocabulary.model';

/**
 * Resolves display labels (Sector/Marca/Tienda for Ventas, Contraparte for Cobranzas) for the
 * current tenant. See resolveDimensionLabel (tenant-vocabulary.utils.ts) for the fallback chain
 * itself -- this service only binds it to the current mock tenant and the retail preset.
 */
...
export class TenantVocabularyService {
  ...
  labelFor(dimension: DimensionKey): string {
